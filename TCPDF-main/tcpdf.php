...
class TCPDF {
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		$ratioHW = ((($row * $mh) + $hpad) / (($cols * $mw) + $vpad));
		$ratioWH = ((($cols * $mw) + $vpad) / (($row * $mh) + $hpad));
		if (!$distort) {
				...
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						if (isset($dom[$parentid]['attribute']['rowspan'])) {
							$rowpan = intval($dom[$parentid]['attribute']['rowspan']);
						} else {
							$rowpan = 1;
						}
						...
						if ($rowpan > 1) {
							$trsid = array_push($dom[$table_el]['rowspans'], array('trid' => $trid, 'rowspan' => $rowpan, 'mrowspan' => $rowpan, 'colspan' => $colspan, 'startpage' => $this->page, 'startcolumn' => $this->current_column, 'startx' => $this->x, 'starty' => $this->y));
						}
