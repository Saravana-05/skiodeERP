...
<div class="bg_aliceblue p-3 m-1 pt-0 reportContent">
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center p-2 mb-3 headerDiv">
		<h4 class="mb-0">Job Card Report</h4> 			
	</div>
	<form>
	<div>
		 <label for="fromDtTxt">From :</label>
			<input type="text" autocomplete="off" class="datepicker"  id="fromDtTxt" name="fromDtTxt"  placeholder="dd-MMM-yyyy" value="<?php echo date("d-m-Y");?>"="readonly"  />
		 <label for="toDtTxt">To : </label>
			<input type="text" autocomplete="off" class="datepicker"  id="toDtTxt" name="toDtTxt"  placeholder="dd-MMM-yyyy" value="<?php echo date("d-m-Y");?>"="readonly" />
...
function printJobCardDiv(divId) {  
	...
	header=header+'<div style="text-align:center;font-size:1.2rem;font-weight:bold;">JOB CARD REPORTS</div>';
...
function fnSearch()
{
	...
	$.ajax({
		type: "POST",
		url: "api/get_jobcard_report.php", 
		data:{
			"from":from,
			"to":to,
