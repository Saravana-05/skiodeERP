...
class MatrixFunctions
{
     ...
     *
     * @param mixed $row the number of rows to return, defaults to 1
     ...
     */
    public static function sequence(mixed $row = 1, mixed $columns = 1, mixed $start = 1, mixed $step = 1): string|array
    {
        try {
            $row = (int) Helpers::validateNumericNullSubstitution($row, 1);
            Helpers::validatePositive($row);
        ...
        if ($step === 0) {
            return array_chunk(
                array_fill(0, $row * $columns, $start),
                max($columns, 1)
            );
        }

        return array_chunk(
            range($start, $start + (($row * $columns - 1) * $step), $step),
            max($columns, 1)
        );
    }
