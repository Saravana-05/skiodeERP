...
class Content extends WriterPart
{
    ...
    private function writeRows(XMLWriter $objWriter, Worksheet $sheet, int $sheetIndex): void
    {
        ...
        foreach ($row as $row) {
            ...
            $cellIterator->rewind();
            $rowtyleExists = $sheet->rowDimensionExists($row->getRowIndex()) && $sheet->getRowDimension($row->getRowIndex())->getRowHeight() > 0;
            if ($cellIterator->valid() || $rowtyleExists) {
                if ($spanRow) {
    ...
    private function writeCellMerge(XMLWriter $objWriter, Cell $cell): void
    {
        ...
        $rowpan = ((int) $end[1]) - ((int) $start[1]) + 1;

        $objWriter->writeAttribute('table:number-columns-spanned', (string) $columnSpan);
        $objWriter->writeAttribute('table:number-rows-spanned', (string) $rowpan);
    }
}
