...
class Gnumeric extends BaseReader
{
    ...
    private function loadCell(
        ...
        $row = (int) ($cellAttributes->Rows ?? 0);
        $cols = (int) ($cellAttributes->Cols ?? 0);
        $type = DataType::TYPE_FORMULA;
        $isArrayFormula = ($row > 0 && $cols > 0);
        $arrayFormulaRange = $isArrayFormula ? $this->getArrayFormulaRange($column, $row, $cols, $row) : null;
        if ($ExprID > '') {
            if (((string) $cell) > '') {
                // Formula
    ...
    private function getArrayFormulaRange(string $column, int $row, int $cols, int $row): string
    {
