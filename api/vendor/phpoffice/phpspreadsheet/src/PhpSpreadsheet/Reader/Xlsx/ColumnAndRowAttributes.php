...
class ColumnAndRowAttributes extends BaseParserClass
{
    ...
    public function load(?IReadFilter $readFilter = null, bool $readDataOnly = false, bool $ignoreRowsWithNoCells = false): void
    {
        ...
        if ($this->worksheetXml->sheetData && $this->worksheetXml->sheetData->row) {
            $rowAttributes = $this->readRowAttributes($this->worksheetXml->sheetData->row, $readDataOnly, $ignoreRowsWithNoCells, $readFilter !== null);
        }
        ...
        foreach ($columnsAttributes as $columnCoordinate => $columnAttributes) {
            if (
                $readFilter === null
                || !$this->isFilteredColumn($readFilter, $columnCoordinate, $rowAttributes)
            ) {
        ...
        $rowAttributesAreSet = [];
        foreach ($rowAttributes as $rowCoordinate => $rowAttributes) {
            if (
                $readFilter === null
                || !$this->isFilteredRow($readFilter, $rowCoordinate, $columnsAttributes)
            ) {
                if (!isset($rowAttributesAreSet[$rowCoordinate])) {
                    ...
                    $rowAttributesAreSet[$rowCoordinate] = true;
                }
            }
        }
    }
    ...
    private function isFilteredColumn(IReadFilter $readFilter, string $columnCoordinate, array $rowAttributes): bool
    {
        foreach ($rowAttributes as $rowCoordinate => $rowAttributes) {
