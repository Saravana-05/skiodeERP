...
if(isset($_GET['sq_no']))
{
	...
	if($query=mysqli_query($connection,$sql))
	{
		if(mysqli_num_rows($query)>0)
		{
			...
			{
				if(!is_null($row["max_invoice_no"]))
				{
					$invoice_no=$row["max_invoice_no"]+1;
				}
			}
		}
	}
...
//compare states and find out whether it belongs to igst or cgst,sgst category
	...
	if($query=mysqli_query($connection,$sql))
	{
		...
		if($diff_gst_percentage_count>0)
		{
			...
			{	
				...
				$ins_gst_sql="INSERT INTO `sales_invoice_gst_details`(`invoice_no`, `hsn_codes`, `before_gst_value`, `cgst`, `sgst`, `igst`, `gst_percentage`) VALUES (".$invoice_no.",'".$hsn_codes."','".$before_gst_value."','".$cgst."','".$sgst."','".$igst."','".$gst_percentage."')";
	...
	// Always preserve the original SQ creator — do not replace with whoever is converting (admin may be converting on behalf of operator)
	$sq_nos_safe = implode(',', array_filter(array_map('intval', explode(',', $sq_no))));
	$orig_si_user = mysqli_real_escape_string($connection, $_SESSION['user_name'] ?? 'system'); // fallback
	if (!empty($sq_nos_safe)) {
		$oc_qry = mysqli_query($connection,
			"SELECT created_by FROM sales_quotation_master WHERE quotation_no IN ($sq_nos_safe) LIMIT 1");
		if ($oc_qry && $oc_row = mysqli_fetch_assoc($oc_qry)) {
			if (!empty($oc_row['created_by'])) $orig_si_user = mysqli_real_escape_string($connection, $oc_row['created_by']);
		}
	}

	$sql = "INSERT INTO `sales_invoice_master`(`invoice_no`, `sale_quotation_nos`, `invoice_dt_tm`, `approximate_amount`, `discount`,inclusive_gst,total_tax,round_off,net_total, `created_by`) VALUES ('".$invoice_no."','".$sq_no."','".$dt."','".$approximate_amount."','".$discount."',".$inclusive_gst.",'".$total_tax."','".$round_off."','".$net_total."','".$orig_si_user."')";
	...
	{ */
		...
		$sql = "update sales_quotation_master set is_this_converted_into_invoice=1,invoice_no=".$invoice_no.",invoice_converted_by='".$orig_si_user."' where quotation_no in (".$sq_no.");";
		mysqli_query($connection,$sql);
	/* } */
	$result=$invoice_no;
		...
		$val_sql="";
		if($sub_total>0)
			$val_sql.="('".$dt."','SALES','Sales Invoice :".$invoice_no." Quote : ".$sq_no."',".$sub_total+$round_off.",'SI_".$invoice_no."','".$customer_code."')";
		if($igst>0)
		{
			if($val_sql!="") $val_sql.=",";
