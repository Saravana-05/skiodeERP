...
//Receipt Voucher
$sql="select * from receipt_voucher where voucher_no=271;";
if($qry=mysqli_query($connection,$sql))
{
	if($row=mysqli_fetch_array($qry))
	{
		...
		$amount=$row["amount"];
	post_journal_double_entry($dt,$customer_code,$account_head,"Receipt Voucher :".$voucher_no,$amount,"RCPTV_".$voucher_no);		
	}
}
//JC
...
//SQ
/*$sql="select * from sales_quotation_master where quotation_no in (143,273,262,416,503)";
if($qry=mysqli_query($connection,$sql))
{
	while($row=mysqli_fetch_array($qry))
	{
...
if($qry=mysqli_query($connection,$sql))
{
	while($row=mysqli_fetch_array($qry))
	{
		post_si_recalc($row["invoice_no"]);
	}
}*/
echo "Done";
function post_si_recalc($pSi_No)
{
	echo $pSi_No."<br>";
	global $connection;
	$invoice_no=$pSi_No;
