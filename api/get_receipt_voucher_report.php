...
if(isset($_POST['type']))
{
	if($_POST['type'] == "get")
	{
		...
		$total_amount=0;
		$sql="SELECT * FROM  receipt_voucher where voucher_date between '".$from."' and '".$to."' ";  
		//echo  "ACH".$account_head."-----" ;
		if($account_head != "All")
		{
			$sql=$sql." and customer_code='".$account_head."'";
		}
		$result="";
		//$result.=$sql;
		...
		if($query=mysqli_query($connection,$sql))
		{
			...
			{ 
				 while($row=mysqli_fetch_array($query)) 
				 {
					...
					$result=$result.'<td>'.$row["customer_name"].'</td>'; 
					...
					$result=$result.'<td style="white-space:nowrap;"><button class="btn btn-sm btn-primary me-1 mt-1" onclick="fnReceiptView(this)" title="View"><span class="material-icons-round" style="font-size:16px;vertical-align:-3px;">visibility</span></button><button class="btn btn-sm btn-danger deleteBtn mt-1" onclick="fnReceiptDelete('.$row["voucher_no"].')" title="Delete"><span class="material-icons-round" style="font-size:16px;vertical-align:-3px;">delete</span></button></td>';
					/*if($row["amount"] >0)
					{
	...
	// Build account_head wise totals from the same query
$cash_online_totals = [];
$sql2 = "SELECT account_head, SUM(amount) as total FROM receipt_voucher WHERE voucher_date BETWEEN '".$from."' AND '".$to."'";
if($account_head != "All") {
    $sql2 .= " AND customer_code='".$account_head."'";
}
...
$result.='</div>';
	...
	$result.='<br> <center><button type="button" class="btn btn-primary btn-sm" onclick="printReceiptVoucherDiv('."'printable_div'".')">PRINT</button><br><br>';
	...
	{
		$dt=date('Y-m-d');
		$sql="INSERT INTO `receipt_voucher`( `voucher_no`, `voucher_date`, `ref_no`, `account_head`, `customer_code`, `customer_name`, `amount`, `description`, `created_on`) VALUES ( '".$_POST["voucher_no"]."','".date("Y-m-d", strtotime($_POST["voucher_date"]))."','".$_POST["ref_no"]."','".$_POST["account_head"]."','".$_POST["customer_code"]."','".$_POST["customer_name"]."','".$_POST["amount"]."','".$_POST["description"]."','".$dt."')";
		 
		mysqli_query($connection,$sql);
/*		$sql="INSERT INTO `journal_details`( `dt`, `achead`, `description`, `amount`, `link_key`) VALUES ";
		$val_sql="";
		$val_sql.="('".date("Y-m-d", strtotime($_POST["voucher_date"]))."','".$_POST["account_head"]."','".$_POST["description"]."','-".$_POST["amount"]."','RCPTV_".$_POST["voucher_no"]."')";
		if($val_sql!="") $val_sql.=",";
		$val_sql.="('".date("Y-m-d", strtotime($_POST["voucher_date"]))."','".$_POST["customer_code"]."','".$_POST["description"]."','".$_POST["amount"]."','RCPTV_".$_POST["voucher_no"]."');";
		$sql.=$val_sql;
		
		mysqli_query($connection,$sql);*/

		post_journal_double_entry($dt,$_POST["customer_code"],$_POST["account_head"],"Receipt Voucher :".$_POST["voucher_no"],$_POST["amount"],"RCPTV_".$_POST["voucher_no"]);		
	...
	{
		delete_journal_entries("RCPTV_".$_POST['v_no']);
		$sql="DELETE FROM  receipt_voucher where voucher_no='".$_POST['v_no']."'";
		mysqli_query($connection,$sql); 
		$result = "success";
	}
}
