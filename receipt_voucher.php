...
<div class="bg_aliceblue p-1 m-1 pt-1 reportContent" >
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center p-1 headerDiv">
		<h5 class="mb-0 ">Receipt Voucher</h5> 			
	</div>
	<?php
	...
	<div class="card mb-3" style="border:0px;" >
		<div class="card-header  text-light " style="border: 1px solid #0fa9c5;background: #0fa9c5 !important;padding-left: 10px;">Receipt Voucher Entry</div>
		<div class="card-body p-2"  style="border: 1px solid #ccc;min-height: max-content;">
			...
			<div class="row mt-1">
				...
				<label for="staticEmail" class="col-md-2 col-form-label">Customer</label>
				<div class="col-md-2">
					<select class="form-select" id="customer_txt" onchange="customer_txt_change()" >
					<option value="Select">Select</option>
					<?php
					if($row=mysqli_query($connection,"select *  from customer_master where customer_type like '%CREDITCUSTOMER%'"))		
					{ 
						while($data=mysqli_fetch_array($row))
						{
	...
	<div class="card mb-3" style="border:0px;" >
		<div class="card-header  text-light " style="border: 1px solid #0fa9c5;background: #0fa9c5 !important;padding-left: 10px;">Receipt Voucher Report</div>
		<div class="card-body p-2"  style="border: 1px solid #ccc; min-height: max-content;">
			<div class="row">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style="border:none;border-radius:12px;overflow:hidden;">
      <div class="modal-header" style="background:#0fa9c5;color:#fff;padding:12px 18px;">
        <h6 class="modal-title mb-0" style="font-weight:700;"><span class="material-icons-round" style="font-size:20px;vertical-align:-4px;margin-right:6px;">receipt_long</span>Receipt Voucher Details</h6>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" style="padding:20px 24px;" id="receiptViewBody"></div>
...
function printReceiptVoucherDiv(divId) {  
	...
	header=header+'<div style="text-align:center;font-size:1.2rem;font-weight:bold;">RECEIPT VOUCHER REPORTS</div>';
...
function fnSearch()
{
	...
	var to = $('#toDtTxt').val();
	var account_head = $('#account_head :selected').val();
	if (!from || !to) {
	...
	$.ajax({
		type: "POST",
		url: "api/get_receipt_voucher_report.php", 
...
function fnReceiptView(btn) {
	...
	html += '<tr><td style="font-weight:700;color:#64748b;">Customer Name</td><td style="font-weight:600;color:#1e293b;">' + customer + '</td></tr>';
	html += '<tr><td style="font-weight:700;color:#64748b;">Ref No</td><td style="font-weight:600;color:#1e293b;">' + (refNo || '-') + '</td></tr>';
	html += '<tr><td style="font-weight:700;color:#64748b;">Amount</td><td style="font-weight:700;color:#16a34a;font-size:1.1rem;">₹ ' + amount + '</td></tr>';
	html += '<tr><td style="font-weight:700;color:#64748b;">Comments</td><td style="font-weight:500;color:#1e293b;">' + (comments || '-') + '</td></tr>';
	html += '</table>';
	$('#receiptViewBody').html(html);
	new bootstrap.Modal(document.getElementById('receiptViewModal')).show();
}

function fnReceiptDelete(pId)
{
