		...
		<div class=" row">
			<div class="col-md-6" style="align-content: center;">
				<select class="form-select" id="stockOpt">
				  ...
				  <?php
				  if($row=mysqli_query($connection,"select *  from papers_stock_master   where  is_active=1"))		
					{ 
						while($data=mysqli_fetch_array($row))
						{
							echo '<option value="'.$data["papers_stock_id"].'">'.$data["papers_stock_name"].'</option>';           
						}
					}
				  ?>
				</select> 
	...
	<div class="col-md-8 p-2 machineDiv bg_blue">
		...
		<div class=" row">
			<div class="col-md-2" style="align-content: center;">
				<input type="text" autocomplete="off" class="form-control numericOnly" id="firstCopyTxt" name="firstCopyTxt" />
			</div> 	  
			<div class="col-md-2" style="align-content: center;">
				<input type="text" autocomplete="off" class="form-control numericOnly" id="addlCopyTxt" name="addlCopyTxt" disabled /> 
			</div> 	  
			<div class="col-md-2" style="align-content: center;">
				<select class="form-select" id="fbCopyTxt" name="fbCopyTxt">
				  <option value="Yes">Yes</option>
				  <option value="No">No</option>
				</select>  
			</div> 	  
			<div class="col-md-2" style="align-content: center;">
				<input type="text" autocomplete="off" class="form-control numericOnly" id="sheetsTxt" name="sheetsTxt" /> 
			</div> 	  
			<div class="col-md-2" style="align-content: center;">
				<select class="form-select" id="machineOpt">
				  <option value="0">-Select-</option>
				  <?php
				  if($row=mysqli_query($connection,"select *  from machine_master   where  is_active=1 and sales_subtype_name='papers'"))		
					{ 
						while($data=mysqli_fetch_array($row))
						{
...
<div id="footerDiv" class="row m-0 p-0">
	...
	<div class="col-5">
		<input type="button" class="btn btn-sm btn-darkpurple" id="saveBtn" name="saveBtn" value="SAVEdfg" style="border: 2px solid #fff;" onclick="saveJobCard()"/>
