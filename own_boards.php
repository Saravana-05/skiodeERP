		...
		<div class=" row">
			<div class="col-md-6" style="align-content: center;">
				<select class="form-select" id="stockOpt">
				  ...
				  <?php
				  if($row=mysqli_query($connection,"select *  from boards_stock_master   where  is_active=1"))		
					{ 
						while($data=mysqli_fetch_array($row))
						{
							echo '<option value="'.$data["boards_stock_id"].'">'.$data["boards_stock_name"].'</option>';           
						}
					}
				  ?>
				</select> 
		...
		<div class=" row">
			...
			<div class="col-md-2" style="align-content: center;">
				<select class="form-select" id="machineOpt">
				  <option value="0">-Select-</option>
				  <?php
				  if($row=mysqli_query($connection,"select *  from machine_master   where  is_active=1 and sales_subtype_name='boards'"))		
					{ 
						while($data=mysqli_fetch_array($row))
						{
