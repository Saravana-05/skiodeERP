...
<div class="container ">
  <h5 class="mb-1 mt-1" style="width:75%;float:left;">User Master</h5>

  <button class="btn btn-sm btn-primary mt-1" id="addUserBtn" style="float:right;">Add User</button>
...
$(document).ready(function () {
  ...
  $('#userForm').submit(function (e) {
    ...
    $.ajax({
      url: `api/crud_user_api.php?action=${action}`,
      method: 'POST',
  ...
  $('#userDiv').on('click', '.editBtn', function () {
    const row = $(this).closest('tr');
	 
    var code = row.data('id'); 
	 
	$('#actionId').val('update');
	$.get('api/crud_user_api.php?action=editrow&code='+code, function (data) {
      let rows = ''; 
  ...
  $('#userDiv').on('click', '.deleteBtn', function () {
    if (!confirm('Delete this customer?')) return;
    const id = $(this).closest('tr').data('id');
    $.get(`api/crud_user_api.php?action=delete&code=${id}`, function () {
      loadUsers();
    });
  });
});
  ...
  function loadUsers() {
    $.get('api/crud_user_api.php?action=read', function (data) {
      let rows = '';
	   $('#userDiv').html('');
